  	...
  	<div class="main" align="center">
		...
		<form id="myForm" action="foodList" method="POST">
		   <table  width="1000" align="center" border="1" cellpadding="30" cellspacing="0" style="text-align: center;">
			  ...
			  <!-- 아작스를 활용해서 FOOD 디비에 저장된 음식목록을 foodList를 활용해서 1페이지 목록의 형태로 불러온다. -->
			  <tbody id="ajaxTable">
		      <c:set var="list" value="${foodList.list}"/>
			     <c:forEach var="vo" items="${list}">
				    <tr>
				       <td class="text-center">${vo.idx}</td>
				       <td class="text-center">${vo.foodName}</td>
				       <td class="text-center">${vo.kcal}</td>
			      	   <td class="text-center">${vo.carbs}</td>
			      	   <td class="text-center">${vo.protein}</td>
			      	   <td class="text-center">${vo.fat}</td>
					 ...
					 <td colspan="7" align="center">
						...
						<c:forEach var="i" begin="${foodList.startPage}" end="${foodList.endPage}" step="1">
							<c:if test="${foodList.currentPage == i}">
								<button type='button' 
										disabled='disabled' 
										style="background: none; border: 0; cursor: pointer; font-size: 25pt;">${i}
								</button>
							</c:if>
							<c:if test="${foodList.currentPage != i}">
								<button 
									...
									style="background: none; border: 0; cursor: pointer;"
								><span style="background: lavender; font-size: 25pt;">${i}</span> <!-- 몇 페이지인지 버튼 -->
								</button>
							</c:if>
						</c:forEach>
						<!-- 10페이지 뒤로 -->
