...
<body>
<div class="container">
	<div class="header"><jsp:include page="./headerAfter.jsp"/></div>
	   <div class="main" align="center">
		  <div class="dietContent_title"><span style="background: #fafcd9; font-size: 30pt; font-weight: 900;">음식 검색</span></div><br/>
			<form id="myForm" action="foodList" method="POST">
				<table width="1000" align="center" border="1" cellpadding="30" cellspacing="0" style="text-align: center;">
					<thead>
						<tr style="height: 60px;">
						   	<th colspan="7" class="text-center">
			        			...
			        			<button class='button button1' type="submit" style="width: 100px; height: 30px; background: none; border: 0; cursor: pointer;">
					...
					<fmt:requestEncoding value="UTF-8"/>
				  	<!-- request 영역에 저장한 foodList에서 1페이지 분량의 글을 꺼내온다. -->
					<tbody id="ajaxTable">
			   		<c:set var="list" value="${foodList.list}"/>
						<c:forEach var="fvo" items="${list}">
							<tr>
							   <td class="text-center">${fvo.idx}</td>
							   <td class="text-center">${fvo.foodName}</td>
							   <td class="text-center">${fvo.kcal}</td>
							   <td class="text-center">${fvo.carbs}</td>
							   <td class="text-center">${fvo.protein}</td>
							   <td class="text-center">${fvo.fat}</td>
						...
						<td colspan="7" align="center">
							...
							<c:forEach var="i" begin="${foodList.startPage}" end="${foodList.endPage}" step="1">
								<c:if test="${foodList.currentPage == i}">
									<button type='button' disabled='disabled' style="background: none; border: 0; cursor: pointer; font-size: 25pt; margin-right: 10px;">
										${i}
									</button>
								</c:if>
								<c:if test="${foodList.currentPage != i}">
									<button 
										...
										style="background: none; border: 0; cursor: pointer; margin-right: 10px;">
  	...
  	<div class="footer"><jsp:include page="./footerAfter.jsp"/></div>
</div>
</body>
</html>